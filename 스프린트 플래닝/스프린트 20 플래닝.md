...
### 1. 운동 메이트를 찾는 유저로서 다른 회원의 프로필에서 회원의 선호 운동을 확인하고 싶다. 그래서 나와 같은 종목을 즐기는 파트너를 찾고 싶다. (4)

- 개발 및 학습 목록
    - 백엔드
        1. 선호 운동 리스트 수정 PATCH
            1. 수정할 선호운동 리스트
            2. 날짜, 수준
        2. 선호 운동 리스트 삭제 DELETE
            1. 논리적 삭제
            2. 리스트로 삭제
    - 클라이언트
        - API 연결 및 테스트
...
### 3. **채팅 사용자로서 채팅방에 들어왔을 때 대화 내용을 실시간으로 확인하고 싶다. 그래서 별도의 로딩 기다림 없이 바로 대화를 이어가는 가치를 얻는다. (9)**
